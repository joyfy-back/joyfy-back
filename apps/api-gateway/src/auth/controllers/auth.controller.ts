import { Body, Controller, HttpCode, HttpException, HttpStatus, Post, Request, Res } from "@nestjs/common";
...
export class AuthController {
    constructor(protected commandBuse: CommandBus, protected authService: AuthService, protected emaiService: EmailService) { }
    ...
    async login(@Body() inputModul: UserLoginInputModule, @Res() res: Response, @Request() req) {
        const checkCreadentlais = await this.authService.checkCreadentlais(inputModul.Email, inputModul.Password)
    ...
    async logout(@Request() req) {
        const result = await this.authService.checkValidateUserSessionByRefreshToken(req.cookies.refreshToken)
        ...
        await this.commandBuse.execute(
            new DeleteSeissionCommand(
                result.data[0].userId,
                result.data[0].deviceId
            )
        )
    ...
    @ApiResponse({ status: 204, description: 'Password recovery email sent successfully.' })
    @ApiResponse({ status: 400, description: 'Bad request.' })
    ...
    async newPassword(@Body() inputNewData: NewPasswordInputModele) {
        const result = await this.authService.checkPasswordRecovery(inputNewData.recoveryCode);
