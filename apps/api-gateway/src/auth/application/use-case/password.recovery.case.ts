...
export class PasswordRecoveryUseCase implements ICommandHandler<PasswordRecoveryCommand> {
  ...
  async execute(inputModul: EmailInputModele): Promise<Result> {
    try {
      ...
      const result = await this.authRepository.postPasswordRecoveryCode(passwordRecoveryCode, inputModul.email);

      if (!result.success) {
        throw new Error()
      }
      this.emailService.sendEmail("null", inputModul.email, passwordRecoveryCode);
